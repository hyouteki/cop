...
// MY COMPILE TIME CONSTANTS DECLARATION START
#define UNMARK 0
// END
...
// MY STATICS, STRUCTS & APIS START
typedef struct ObjHeaderList {
	struct ObjHeader *ObjHeader;
	struct ObjHeaderList *Next;
} ObjHeaderList;

static ObjHeaderList *ObjHeaders = NULL; // Scanner list

static void insertFrontObjHeaders(ObjHeader *);
static ObjHeader *popFrontObjHeaders();

static void insertFrontObjHeaders(ObjHeader *ObjHead) {
	ObjHeaderList *L = malloc(sizeof(ObjHeaderList));
	if (L == NULL) {
		printf("ERROR: Unable to allocate memmory for ObjHeaderList object\n");
		exit(0);
	}
	L->ObjHeader = ObjHead;
	L->Next = ObjHeaders;
...
static ObjHeader *popFrontObjHeaders() {
	if (ObjHeaders == NULL) return NULL;
	ObjHeader *ObjHead = ObjHeaders->ObjHeader;
	ObjHeaders = ObjHeaders->Next;
	return ObjHead;
}

// Forward declaration of `scanRoots` API
static void scanRoots(unsigned char *, unsigned char *);
// END
...
void scanner()
{
	while (ObjHeaders != NULL) {
		ObjHeader *ObjHead = popFrontObjHeaders();
		unsigned char *Top = (unsigned char *) ObjHead;
		scanRoots(Top + OBJ_HEADER_SIZE, Top + ObjHead->Size);
	} 
}
...
void sweep()
{
	SegmentList *Itr = Segments;
	while (Itr != NULL) {
		Segment *Seg = Itr->Segment;
		Itr = Itr->Next;

		unsigned char *Ptr = (unsigned char *) getDataPtr(Seg);
		unsigned char *Bottom = (unsigned char *) getAllocPtr(Seg);

		while (Ptr <= Bottom - 8) {
			ObjHeader *Header = (ObjHeader *) Ptr;
			assert(((ulong64) Header & (PAGE_SIZE-1)) == 0 && "Header is should be PAGE_SIZE aligned");

			ulong64 IncSize = Header->Size;
			if (Header->Size < PAGE_SIZE) {
				ulong64 AllocSize = Header->Size + OBJ_HEADER_SIZE;
				unsigned int NumPages = AllocSize/PAGE_SIZE;
				if (AllocSize%PAGE_SIZE) ++NumPages;
				IncSize = NumPages*PAGE_SIZE;
			}
...
static void scanRoots(unsigned char *Top, unsigned char *Bottom)
{
	...
	ulong64 DataPtrs[NumSegments];
	ulong64 AllocPtrs[NumSegments];
	Itr = Segments;
	int i = 0;
	while (Itr != NULL) {
		DataPtrs[i] = (ulong64) getDataPtr(Itr->Segment);
		AllocPtrs[i] = (ulong64) getAllocPtr(Itr->Segment);
		++i;
		Itr = Itr->Next;
	}
	
	for (unsigned char* Ptr = Top; Ptr <= Bottom - 8; ++Ptr) {
		ulong64 ValAtPtr = *(ulong64 *)Ptr;

		for (int i = 0; i < NumSegments; ++i) { 
			if (DataPtrs[i] + OBJ_HEADER_SIZE <= ValAtPtr && ValAtPtr < AllocPtrs[i]) {
				ObjHeader *Header = (ObjHeader *)(ValAtPtr - OBJ_HEADER_SIZE);
				if (Header->Status == UNMARK) {
					Header->Status = MARK;
					insertFrontObjHeaders(Header);
				}
			}
		}
	}
